...
public:
	...
	MIPRTPReceiveMessage(
		RTPPacket *pPack, 
		const uint8_t *pCName, 
		size_t cnameLength, 
		bool deletePacket = true, 
		RTPSession *pSess = 0) : 
	MIPMessage(MIPMESSAGE_TYPE_RTP, MIPRTPMESSAGE_TYPE_RECEIVE), 
		m_jitter(0)
	{ 
		m_deletePacket = deletePacket; 
		m_pPack = pPack; 
		if (cnameLength > MIPRTPMESSAGE_MAXCNAMELENGTH) 
			m_cnameLength = MIPRTPMESSAGE_MAXCNAMELENGTH; 
		else 
			m_cnameLength = cnameLength; 
		
		if (cnameLength > 0) 
			memcpy(m_cname,pCName,m_cnameLength); 
	...
	{ 
		...
		{ 
			if (m_pSession) 
				m_pSession->DeletePacket(m_pPack); 
			else 
				delete m_pPack; 
		} 
	}
