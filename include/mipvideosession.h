...
public:
	...
	enum SessionType 
	{ 
		InputOutput, 	/**< 支持视频采集+回放 */
		OutputOnly 	    /**< 仅接收视频，不采集本地视频 */
	};
	...
	/** Returns the avcodec frame rate (default: 25.0). */
	real_t getAvcodecFrameRate() const				{ return m_avcodec_frame_rate; }
	...
	/** Returns flag indicating if a Win32 output component will be used or if a
	 *  video frame storage component will be used (default: \c true).
	 */
	bool getUseWin32Output() const							{ return m_win32output; }

	/** 获取Windows下的视频数据处理类 */
	MIPWin32OutputEvent *getMIPWin32OutputEvent() const      { return m_pMIPWin32OutputEvent; }
	...
	/** Sets the avcodec frame rate. */
	void setAvcodecFrameRate(real_t r) 					{ m_avcodec_frame_rate = r; }
	...
	/** Sets flag if Win32 output component should be used or a if video frame
	 *  storage component should be used.
	 */
	void setUseWin32Output(bool f)							{ m_win32output = f; }

	/** 设置Windows下的视频数据处理类 */
	void setMIPWin32OutputEvent(MIPWin32OutputEvent* pMIPWin32OutputEvent)
	{ 
...
public:
	 ...
	 *  \param pm_Params Session parameters.
	 ...
	 *                     adjusted.
 	 */
	bool init(
		const MIPVideoSessionParams *pm_Params = 0, 
		MIPRTPSynchronizer *pSync = 0, 
		RTPSession *pRTPSession = 0);
	
	/** 销毁此回话 */
	bool destroy();

	/** 设置RTP数据包发送目标 */
	bool addDestination( char* lp_addr,int port );
	...
	/** 设置接收模式(AcceptAll,AcceptSome,IgnoreSome)
